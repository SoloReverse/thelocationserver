...
type CarQuery struct {
	...
	withCarsowned      *UserQuery
	modifiers          []func(*sql.Selector)
	loadTotal          []func(context.Context, []*Car) error
	withNamedCarsowned map[string]*UserQuery
...
// QueryCarsowned chains the current query on the "carsowned" edge.
func (cq *CarQuery) QueryCarsowned() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, car.CarsownedTable, car.CarsownedPrimaryKey...),
		)
...
func (cq *CarQuery) Clone() *CarQuery {
	...
	return &CarQuery{
		...
		withCarsowned: cq.withCarsowned.Clone(),
...
// WithCarsowned tells the query-builder to eager-load the nodes that are connected to
// the "carsowned" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CarQuery) WithCarsowned(opts ...func(*UserQuery)) *CarQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCarsowned = query
	return cq
}
...
func (cq *CarQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Car, error) {
	...
	if query := cq.withCarsowned; query != nil {
		if err := cq.loadCarsowned(ctx, query, nodes,
			func(n *Car) { n.Edges.Carsowned = []*User{} },
			func(n *Car, e *User) { n.Edges.Carsowned = append(n.Edges.Carsowned, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedCarsowned {
		if err := cq.loadCarsowned(ctx, query, nodes,
			func(n *Car) { n.appendNamedCarsowned(name) },
			func(n *Car, e *User) { n.appendNamedCarsowned(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CarQuery) loadCarsowned(ctx context.Context, query *UserQuery, nodes []*Car, init func(*Car), assign func(*Car, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Car)
	nids := make(map[uuid.UUID]map[*Car]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(car.CarsownedTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(car.CarsownedPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(car.CarsownedPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(car.CarsownedPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "carsowned" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
// WithNamedCarsowned tells the query-builder to eager-load the nodes that are connected to the "carsowned"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (cq *CarQuery) WithNamedCarsowned(name string, opts ...func(*UserQuery)) *CarQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedCarsowned == nil {
		cq.withNamedCarsowned = make(map[string]*UserQuery)
	}
	cq.withNamedCarsowned[name] = query
	return cq
}
