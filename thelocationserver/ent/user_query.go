...
func (uq *UserQuery) QueryCars() *CarQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, user.CarsTable, user.CarsPrimaryKey...),
		)
...
func (uq *UserQuery) loadCars(ctx context.Context, query *CarQuery, nodes []*User, init func(*User), assign func(*User, *Car)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.CarsTable)
		s.Join(joinT).On(s.C(car.FieldID), joinT.C(user.CarsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.CarsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.CarsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Car](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
